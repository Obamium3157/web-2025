...
function connectToDatabase1() : PDO {
    $dsn = 'mysql:host=' . DB_HOST . ';dabname=' . DB_NAME;
    return new PDO($dsn, DB_USER, DB_PASSWORD);
}

function getPostsFromDatabase1(PDO $connection, int $limit = 100) : array {
    $query = <<<SQL
        SELECT
          title, image
        FROM
...
function savePostToDatabase1(PDO $connection, string $title, string $image): bool {
