...
function renderProfile(array $user, array $posts, array $images) {
    ...
    <img class="content__profile-image" src="../data/users_data/<?php echo $user['user_id'] ?>/<?php echo $user['profile_picture']?>" alt="Аватар пользователя">
...
function renderPostImage(array $post, array $images) {
    foreach ($images as $image) {
?>
        <img class="posts__post-image" src="../data/users_data/<?php echo $post['user_id'] ?>/posts/<?php echo $image['filename']?>" alt="Картинка в посте">
<?php
    }
}
?>
